...
import { Eye, Pencil, Globe, FilePlus, Plus, Search, Minus, Check, ChevronDown } from 'lucide-react'
...
import { useGetDocumentsStatistics, getCountByStatus } from '@/hooks/use-documents-statistics'
import { useGetAllDocuments, type AdminDocumentRow } from '@/hooks/use-get-all-documents'
import {
  DOCUMENT_STATUSES,
  documentStatusToApi,
  type DocumentStatus,
...
function formatModified(date: Date): { line1: string; line2: string } {
  ...
  if (date >= subDays(new Date(), 7)) return { line1: format(date, 'EEEE'), line2 }
  return { line1: format(date, 'MMM d, yyyy'), line2 }
}

function ownerDisplay(createdBy: string | null, fullName: string | null): string {
...
export default function AdminPage() {
  ...
  const { data: statistics, isLoading: statsLoading } = useGetDocumentsStatistics()
  const [activeTab, setActiveTab] = useState<DocumentStatus>('inProgress')
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState('')
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
  const [selectedMaterialsMap, setSelectedMaterialsMap] = useState<Map<string, number>>(new Map())
  ...
  const { data: viewDocument, isLoading: viewDocumentLoading } = useGetDocumentDetails(viewDocumentId)
  const { data: editDocument, isLoading: editDocumentLoading } = useGetDocumentDetails(editDocumentId)
  ...
  const getCount = (status: DocumentStatus) => getCountByStatus(statistics, status)

  const visibleIds = useMemo(() => paginatedDocs.map((d) => d.id), [paginatedDocs])
      ...
      },
      {
        onSuccess: (data) => {
          ...
          const failureDetails =
            failureCount > 0
              ? failedItems.map((item) => getFailedItemDisplayMessage(item)).join('\n')
              : ''
          ...
          if (successCount > 0 && failureCount > 0) {
            ...
            setResultModal({
              ...
              content: failureDetails ? `${summary}\n\n${failureDetails}` : summary,
              isError: true,
            })
            ...
            setResultModal({
              ...
              content: t('admin.documents.transitionSuccess', { count: successCount }),
            })
          }
        },
        onError: () => {
      ...
      <div className="min-h-0 min-w-0 flex-1 space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <TabsList className="flex flex-wrap h-auto gap-1 bg-transparent p-0 border-b border-border rounded-none w-full sm:w-auto">
            {DOCUMENT_STATUSES.map((status) => (
              ...
              >
                {t(`admin.documents.${status}`)} ({statsLoading ? '—' : getCount(status)})
              </TabsTrigger>
            ))}
          </TabsList>
        </Tabs>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex min-w-0 flex-wrap items-center gap-3 rounded-lg border border-border bg-surface px-4 py-2">
              <button
                type="button"
                role="checkbox"
                aria-checked={headerCheckedState === true ? 'true' : headerCheckedState === 'indeterminate' ? 'mixed' : 'false'}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  {DOCUMENT_STATUSES.filter((status) => status !== activeTab).map((status) => (
                    <DropdownMenuItem
                      key={status}
                      onClick={() => {
        ...
        <Table className="border-0 min-w-[900px] table-auto">
          <TableHeader className="[&_tr]:border-0">
            <TableRow className="border-0 bg-surface">
              ...
              <TableHead className="font-bold p-4 min-w-[100px]">{t('admin.documents.reference')}</TableHead>
              <TableHead className="font-bold p-4 min-w-[140px]">{t('admin.documents.titleCol')}</TableHead>
              <TableHead className="font-bold pl-4 pr-8 pt-4 pb-4 max-w-[96px]">{t('admin.documents.type')}</TableHead>
              <TableHead className="font-bold p-4 min-w-[80px]">{t('admin.documents.materials')}</TableHead>
              <TableHead className="font-bold p-4 min-w-[80px]">{t('admin.documents.owner')}</TableHead>
              <TableHead className="font-bold p-4 min-w-[90px]">{t('admin.documents.modified')}</TableHead>
              <TableHead className="font-bold p-4 w-14 shrink-0 whitespace-nowrap text-right" aria-label={t('admin.documents.language')}>
              ...
              paginatedDocs.map((doc, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="max-w-[240px] px-4 py-7">
                      <div className="flex items-center gap-2 min-w-0" title={doc.title ?? undefined}>
                        <MdOutlineTextSnippet className="size-4 shrink-0 text-muted-foreground" />
                        <span className="truncate">{doc.title}</span>
                      </div>
                    </TableCell>
                    <TableCell className="pl-4 pr-8 py-7 max-w-[96px]">
                      <span className="inline-block max-w-[96px] min-w-0 rounded-md bg-surface px-1.5 py-0.5 text-xs truncate" title={doc.type}>
                        {doc.type}
                      </span>
                    </TableCell>
                    <TableCell className="pl-8 pr-4 py-7">{doc.materialsCount}</TableCell>
                    ...
                    <TableCell className="px-4 py-7">
                      <div className="flex flex-col">
                        <span className="text-muted-foreground text-xs">{line1}</span>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        <TooltipProvider delayDuration={0}>
                          <Tooltip>
                            ...
                            <TooltipContent>{t('admin.documents.actionView')}</TooltipContent>
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                type="button"
                                className="p-2 rounded-md text-muted-foreground hover:bg-hover hover:text-foreground"
                                aria-label={t('admin.documents.actionAddMaterials')}
                                onClick={() =>
                            ...
                            <TooltipContent>{t('admin.documents.actionAddMaterials')}</TooltipContent>
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                            ...
                            <TooltipContent>{t('admin.documents.actionEdit')}</TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                    </TableCell>
        ...
        <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              >
                {transitionMutation.isPending ? t('common.loading') : t('common.continue')}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        >
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              <AlertDialogAction
                className={resultModal.isError ? 'bg-destructive text-destructive-foreground hover:bg-destructive/90' : undefined}
